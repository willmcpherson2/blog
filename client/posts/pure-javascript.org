* Pure JavaScript or: How I Learned to Stop Worrying and Love Shallow Copies
...
1. Perform shallow copies via spread syntax ~{ ...data }~ and hope that you've prevented undesirable mutations
...
Can we actually write JavaScript like this? I want to argue that it's possible in many cases. But you will need to reconsider your tooling.

*** fp-ts

[[https://github.com/gcanti/fp-ts][fp-ts]] is a standard-esque library for typed functional programming. If you're going to write immutable JavaScript, you can't rely on the builtin methods. For example, the [[https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array][Array API]] provides a mix of pure and impure methods. Fortunately the [[https://gcanti.github.io/fp-ts/modules/Array.ts.html][fp-ts Array module]] is totally pure and provides way more functionality.

*** ts-pattern

[[https://github.com/gvergnaud/ts-pattern][ts-pattern]] is an exhaustive pattern matching library. Pattern matching is the key to writing pure functions and ts-pattern is even better then some languages that have it built-in.

*** monocle-ts

[[https://github.com/gcanti/monocle-ts][monocle-ts]] is a functional optics library. You might not actually need this. I thought it was the key to nested updates, but I eventually realised that immutable programming unlocks shallow copies.
